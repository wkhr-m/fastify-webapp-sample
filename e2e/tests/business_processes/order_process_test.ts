...
Scenario("ログインし、お弁当を注文し、お弁当を受け取る", async ({ I, utils }) => {
    ...
    // 注文番号を控えておく
    const orderNumber = await I.grabTextFrom('h3');
    ...
    session("お弁当屋さんのブラウザ", () => {
        ...
        // 注文管理画面から注文を引き渡す
        I.click('注文を管理する');
        const itemCoutainer = locate('aside').withText(orderNumber);
        I.click("この注文を引き渡しました", itemCoutainer);
        I.see("引き渡し済みの注文です", itemCoutainer);
    })
});